...
| `panels/CountsPanel.ts` | FastLineRenderableSeries for mRNA/protein counts. Supports composable display modes: `setGeneLayout(stacked/overlaid)` creates per-gene y-axes via `LeftAlignedOuterVerticallyStackedAxisLayoutStrategy`; `setPathDisplay(stacked/overlaid/mean-se)` creates per-path y-axes or shows mean+SE bands. Modes compose into gene×path grid. `setMeanSEData(summary)` renders mean line + SE band per gene. |
