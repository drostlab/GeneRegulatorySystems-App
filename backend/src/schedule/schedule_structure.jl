...
model_label(wrapped::Models.Wrapped) = model_label(Models.describe(wrapped.definition))
model_label(label::Models.Label) = label.label
model_label(::Models.EmptyDescription) = ""
model_label(x) = type_label(x)

function model_label(model::Plumbing.Adjust)
...
function model_label(model::Plumbing.Seed)
    "Seed\n  $(model.seed)"
end

function model_label(desc::Descriptions)
    i = findfirst(d -> d isa Label, desc.descriptions)
    i !== nothing ? model_label(desc.descriptions[i]) : ""
end

"""Human-readable label derived from a model's type name."""
function type_label(x)::String
...
Gene colours are generated per-model using dispatch (`gene_colours_of`).
Returns `(segments, gene_names, gene_colours)` — deduplicated per model_path.
"""
function collect_segments(grs_schedule)::Tuple{Vector{TimelineSegment}, Vector{String}, Dict{String, String}}
    ...
    function dryrun_collector(primitive!, x, Δt; path, into=nothing, _...)
        ...
        label = user_label isa AbstractString ? user_label : model_label(primitive!.f!.model)
        ...
        if model_path ∉ seen_model_paths && isfinite(Δt) && Δt > 0.0
            ...
            for name in gene_names_of(primitive!)
                s = string(name)
                if s ∉ genes_seen
                    push!(genes_seen, s)
                    push!(genes, s)
            ...
            merge!(gene_colours, gene_colours_of(primitive!))
        end
    end
...
"""
function merge_contiguous_segments(segments::Vector{TimelineSegment})::Vector{TimelineSegment}
...
function build_structure_tree(grs_schedule::GRSSchedule)::StructureNode
    return structure_node(grs_schedule.specification, grs_schedule.bindings, grs_schedule.path, grs_schedule.branch)
end

function structure_node(spec::Scope, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    child_path = "$path$(spec.branch ? '/' : '+')"
    merged_bindings = safe_evaluate_bindings(spec, bindings, path)
    child = structure_node_from_step(spec.step, merged_bindings, child_path, spec.branch)
...
function structure_node(spec::List, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    child_prefix = branch ? path : "$path-"
    children = StructureNode[]

    for (i, item_spec) in enumerate(spec.items)
        item_bindings = Scheduling.descended(bindings, i)
        child = structure_node_from_step(item_spec, item_bindings, "$child_prefix$i", false)
        push!(children, child)
    end

    node_type = (branch || any(subtree_has_branch, children)) ? :branch : :sequence
    return StructureNode(type = node_type, execution_path = path, children = children)
end

function structure_node(spec::Each, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    child_prefix = branch ? path : "$path-"
    ...
    for (i, item) in enumerate(items)
        ...
        child = structure_node_from_step(spec.step, item_bindings, "$child_prefix$i", false)
        push!(children, child)
    end
...
function subtree_has_branch(node::StructureNode)::Bool
    node.type == :branch && return true
    any(subtree_has_branch, node.children)
end

function structure_node(spec::Template, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    expanded = try
    ...
    if expanded isa Specifications.Specification
        return structure_node(expanded, bindings, path, branch)
    end

    return StructureNode(type = :leaf, execution_path = path, label = safe_label(expanded))
end

function structure_node(spec::Slice, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    return StructureNode(type = :leaf, execution_path = path, label = "slice")
end

function structure_node(spec::Load, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    return StructureNode(type = :leaf, execution_path = path, label = "load: $(spec.path)")
end

function structure_node(spec, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    return StructureNode(type = :leaf, execution_path = path)
end

function structure_node_from_step(step, bindings::Dict{Symbol, Any}, path::String, branch::Bool)::StructureNode
    if step isa Specifications.Specification
        return structure_node(step, bindings, path, branch)
    end
    return StructureNode(type = :leaf, execution_path = path, label = safe_label(step))
end

function safe_evaluate_bindings(spec::Scope, bindings::Dict{Symbol, Any}, path::String)::Dict{Symbol, Any}
    try
...
safe_label(x::Models.Model) = model_label(x)
safe_label(x::Models.Wrapped) = model_label(x)
safe_label(x::Number) = "step=$x"
safe_label(::Nothing) = ""
safe_label(x) = string(typeof(x))
