...
function unique_model_paths(segments::Vector{TimelineSegment})::Vector{String}
    seen = Set{String}()
    paths = String[]
    for seg in segments
...
function property_signature(properties::Dict{Symbol, Any})::String
    isempty(properties) && return ""
    parts = String[]
    for (k, v) in sort(collect(properties); by = kv -> string(first(kv)))
        push!(parts, "$(String(k))=" * value_signature(v))
    end
    return join(parts, ";")
end

function value_signature(x)::String
    if x isa Dict
        entries = sort(collect(x); by = kv -> string(first(kv)))
        inner = String["$(string(k)):" * value_signature(v) for (k, v) in entries]
        return "{" * join(inner, ",") * "}"
    elseif x isa AbstractVector
        return "[" * join((value_signature(v) for v in x), ",") * "]"
    elseif x isa Tuple
        return "(" * join((value_signature(v) for v in x), ",") * ")"
    elseif x isa Symbol
        return ":" * String(x)
...
function link_id(l::NetworkRepresentation.Link)::String
